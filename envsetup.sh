...
function settitle()
{
    if [ "$STAY_OFF_MY_LAWN" = "" ]; then
        ...
        if [ -z "$apps" ]; then
            export PROMPT_COMMAND="echo -ne \"\033]0;[${arch}-${product}-${variant}] twisted@buildbox: ${PWD}\007\""
        else
            export PROMPT_COMMAND="echo -ne \"\033]0;[$arch $apps $variant] twisted@buildbox}: ${PWD}\007\""
        fi
    fi
}
